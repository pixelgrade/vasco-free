 ...
 * So we add a transient which will be handled by the `vasco_admin_redirect_to_pixcare_install_once` action
 */
function vasco_force_redirect_to_pixcare_install_once() {
	...
	if ( empty( $plugin_version ) ) {
		set_transient( '_vasco_activation_redirect', 1 );
	}
}
add_action( 'after_switch_theme', 'vasco_force_redirect_to_pixcare_install_once' );

/**
 *  * Redirect the admin to the pixcare install page once only if the plugin is missing
 */
function vasco_admin_redirect_to_pixcare_install_once() {
	if ( ! get_transient( '_vasco_activation_redirect' ) ) {
		return;
	}
	delete_transient( '_vasco_activation_redirect' );
...
add_action( 'admin_init', 'vasco_admin_redirect_to_pixcare_install_once' );

function vasco_add_pixcare_install_page() {
	add_theme_page(  '', '', 'manage_options', 'pixelgrade_care-install', null );
}
add_action( 'admin_menu', 'vasco_add_pixcare_install_page' );

function vasco_pixcare_install_page() {
 ...
 * @return string
 */
function vasco_pixcare_install_page_title( $title ) {
	if ( empty( $_GET['page'] ) || 'pixelgrade_care-install' !== $_GET['page'] ) {
		return $title;
	}
...
add_filter( 'wp_title', 'vasco_pixcare_install_page_title', 10, 1 );
